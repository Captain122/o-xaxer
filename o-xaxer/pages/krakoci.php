...
<head>
    ...
    <meta name="keywords" content="xaxer,krakoci xaxer,online krakel, օնլայն խաղեր" />
    <meta name="description" content="krakoci,shuter,օնլայն խաղեր,խաղացեք անվճար,կրակոցի flash ,html5 խաղերը online-xaxer.xyz կայքում">
...
<div class="container">
    <div class="row">
        <?php require_once('/home/wm65389/domains/online-xaxer.xyz/public_html/SIUD/select_krakoci.php') ?>
        <?php foreach ($araj6 as $post6) {?>
            <?php $k = '../';
            $findme   = 'https';
            $pos = strpos($post6['img'], $findme);
            if($pos !== false){
                $k ='';
            }
            ?>
            <div class="col-md-3" >
                <div style="text-align: center; background-color: white; margin:8% " >
                    <h5><?echo $post6['name']?></h5>
                    <div><img src="<?echo $k?><?echo $post6['img']?>" style="width: 180px"></div>
                    <div><?echo $post6['preview']?></div>
                    <a href="../game.php?xax_id=<?echo $post6['link']?>&nkar=<?echo $post6['img']?>">ԽԱՂԱԼ ՀԻՄԱ --></a>
                </div>
            </div>
        <? } ?>
</div>
