...
<body>
<nav class="navbar navbar-light" style="background-color:yellowgreen;">
    <div class="container">
...
<div class="container">
    <div class="row">
        <?php require_once('/home/wm65389/domains/online-xaxer.xyz/public_html/SIUD/select_zombi.php') ?>
        <?php foreach ($araj5 as $post5) {?>
            <?php $k = '../';
            $findme   = 'https';
            $pos = strpos($post5['img'], $findme);
            if($pos !== false){
                $k ='';
            }
            ?>
            <div class="col-md-3" >
                <div style="text-align: center; background-color: white; margin:8% " >
                    <h5><?echo $post5['name']?></h5>
                    <div><img src="<?echo $k?><?echo $post5['img']?>" style="width: 180px"></div>
                    <div><?echo $post5['preview']?></div>
                    <a href="../game.php?xax_id=<?echo $post5['link']?>&nkar=<?echo $post5['img']?>">ԽԱՂԱԼ ՀԻՄԱ --></a>
                </div>
            </div>
        <? } ?>
</div>
