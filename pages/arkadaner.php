...
<head>
    ...
    <meta name="keywords" content="xaxer,arkada ,sexani xax,arcade,օնլայն խաղեր" />
    <meta name="description" content="arkada xaxer,arcade games,օնլայն խաղեր,խաղացեք անվճար ու երկար,sexani,arkadaner,flash ,html5 խաղերը online-xaxer.xyz կայքում">
...
<div class="container">
    <div class="row">
        <?php require_once('/home/wm65389/domains/online-xaxer.xyz/public_html/SIUD/select_arkada.php') ?>
        <?php foreach ($araj1 as $post1) {?>
            <?php $k = '../';
            $findme   = 'https';
            $pos = strpos($post1['img'], $findme);
            if($pos !== false){
                $k ='';
            }
            ?>
            <div class="col-md-3" >
                <div style="text-align: center; background-color: white; margin:8% " >
                    <h5><?echo $post1['name']?></h5>
                    <div><img src="<?echo $k?><?echo $post1['img']?>" style="width: 180px"></div>
                    <div><?echo $post1['preview']?></div>
                    <a href="../game.php?xax_id=<?echo $post1['link']?>&nkar=<?echo $post1['img']?>">ԽԱՂԱԼ ՀԻՄԱ --></a>
                </div>
            </div>
        <? } ?>
</div>
